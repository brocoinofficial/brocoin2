...
from test_framework.util import assert_equal, assert_raises_rpc_error, bronee_round
...
def make_utxo(node, amount, confirmed=True, scriptPubKey=CScript([1])):
    ...
    """
    fee = 1*COIN
    while node.getbalance() < bronee_round((amount + fee)/COIN):
        node.generate(100)

    new_addr = node.getnewaddress()
    txid = node.sendtoaddress(new_addr, bronee_round((amount+fee)/COIN))
...
class ReplaceByFeeTest(BrocoinTestFramework):
    ...
    def test_doublespend_chain(self):
        ...
        # child fees - 40 BRON - so this attempt is rejected.
        dbl_tx = CTransaction()
