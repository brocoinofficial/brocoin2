This directory contains the BrocoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Brocoin UI.
...
ClientModel is used by the main application `brocoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/brocoin/brocoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a brocoin:... link from a browser).
...
* UI elements like BrocoinAmountField, which inherit from QWidget.
* `brocoinstrings.cpp`: automatically generated
* `brocoinunits.(h/cpp)`: BRON / mBRON / etc handling
* `callback.h`
...
* don't change `local/brocoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "brocoin-qt" under "Run", which is where you can also add command line arguments)
