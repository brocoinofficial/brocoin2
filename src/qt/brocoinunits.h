...
#ifndef BROCOIN_QT_BROCOINUNITS_H
#define BROCOIN_QT_BROCOINUNITS_H
...
/** Brocoin unit definitions. Encapsulates parsing and formatting
   and serves as list model for drop-down selection boxes.
*/
class BrocoinUnits: public QAbstractListModel
{
    Q_OBJECT

public:
    explicit BrocoinUnits(QObject *parent);

    /** Brocoin units.
      @note Source: https://en.brocoin.it/wiki/Units . Please add only sensible ones
     */
    enum Unit
    {
        BRON,
    ...
    //! Number of Bronees (1e-8) per unit
    ...
    //! Return maximum number of base units (Bronees)
    static CAmount maxMoney();

private:
    QList<BrocoinUnits::Unit> unitlist;
};
typedef BrocoinUnits::Unit BrocoinUnit;

#endif // BROCOIN_QT_BROCOINUNITS_H
