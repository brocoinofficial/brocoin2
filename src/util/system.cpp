...
const char * const BROCOIN_CONF_FILENAME = "brocoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "brocoind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //brocoin-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Brocoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Brocoin
    // Mac: ~/Library/Application Support/Brocoin
    // Unix: ~/.brocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Brocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Brocoin";
#else
    // Unix
    return pathRet / ".brocoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BROCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Brocoin Core copyright is not removed by accident
    if (copyright_devs.find("Brocoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Brocoin Core developers";
    }
