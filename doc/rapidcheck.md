# RapidCheck property-based testing for Brocoin Core

## Concept

Property-based testing is experimentally being added to Brocoin Core with
...
If RapidCheck is installed, Brocoin Core will automatically run the
...
3. Configure Brocoin Core with RapidCheck.

    `cd` to the directory of your local brocoin repository and run
...
4. Build Brocoin Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_brocoin-crypto_gen.o
    CXX      test/test_brocoin-key_properties.o
    ```
...
That's it! You are now running property-based tests in Brocoin Core.
